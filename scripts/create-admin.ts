...
async function main() {
  ...
  try {
    const login = await prompt.question("Identifiant admin : ");
    ...
    const admin = await createInitialAdmin(login, password);
    process.stdout.write(`Compte admin « ${admin.login} » créé.\n`);
  } finally {
    output.muted = false;
    prompt.close();
  }
...
main()
  .catch((error: unknown) => {
    if (
      error instanceof AdminCreationError &&
      error.code === "ALREADY_EXISTS"
    ) {
      process.stderr.write("Un compte administrateur existe déjà.\n");
    } else {
